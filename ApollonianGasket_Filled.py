...
OutputFile = 'ApollonianGasket_Filled.ps'



def DrawCircle (file, pos, rad, colour='1 1 1'):

    # print ('  --> DrawCircle ', pos.real, ', ', pos.imag, ',  rad: ', rad)

    file.write (colour + ' setrgbcolor\n');
    ...
    outputTxt += ' '
    outputTxt += str (rad)
    outputTxt += ' 0 360 arc gsave 0 0 0 setrgbcolor 1 setlinewidth stroke grestore fill \n'
...
def RandomColour():
    ...
    #return str (random.randrange(14, 25)/100) + ' ' +\
    #       str (random.randrange(35, 50)/100) + ' ' +\
    #       str (random.randrange(45, 75)/100)
    ...
    return str (np.random.normal(R, R * SD_F)/100) + ' ' + \
           str (np.random.normal(G, G * SD_F)/100) + ' ' + \
           str (np.random.normal(B, B * SD_F)/100) 
...
def RecursivelySolve (b1, z1, b2, z2, b3, z3, b4, z4, level):
    ...
    if (abs(1 / b2prime) > MIN_SIZE):
        DrawCircle (psFile, z2prime, abs(1 / b2prime), RandomColour())

    if (abs(1 / b3prime) > MIN_SIZE):
        DrawCircle (psFile, z3prime, abs(1 / b3prime), RandomColour())

    if (abs(1 / b4prime) > MIN_SIZE):
        DrawCircle (psFile, z4prime, abs(1 / b4prime), RandomColour())
...
def MakeApollonianGasket (b1, z1, b2, z2, b3, z3):
    ...
    # Draw the three base 
    DrawCircle (psFile, z1, abs(1 / b1), '0 0 0')
    DrawCircle (psFile, z2, abs(1 / b2), RandomColour())
    DrawCircle (psFile, z3, abs(1 / b3), RandomColour())

    
    DrawCircle (psFile, z4,      abs(1 / b4), RandomColour())
    DrawCircle (psFile, z4prime, abs(1 / b4prime), RandomColour())
